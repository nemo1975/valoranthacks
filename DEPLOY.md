# Deploy valoranthack.net

Step-by-step guide to deploy the Valorant Hacks static site to **valoranthack.net** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **valoranthack.net** DNS
...
3. Configure build settings:
   - **Project name:** `valorantscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=valoranthacks` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **valoranthack.net** as the primary custom domain on the Pages project.

### Apex (valoranthack.net)
...
- `https://valoranthack.net/es/`
- `https://valoranthack.net/valorant-hacks/`
- `https://valoranthack.net/valorant-aimbot/`
- `https://valoranthack.net/sitemap.xml`
- `https://valoranthack.net/robots.txt`
...
2. **Add property** → choose **Domain** → enter `valoranthack.net`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://valoranthack.net/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `valoranthack.net` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/valorant-hacks/`)
   - Key landing pages (`/valorant-aimbot/`, `/valorant-esp/`, `/valorant-hacks-2026/`, etc.)
...
- [ ] Homepage and `/valorant-hacks/` requested for indexing
