/**
 * SVG overlay generators for Valorant hacks marketing images.
 * Composited on top of IGN Valorant gameplay screenshots.
 */
...
export function modMenuOverlay(w, h) {
	...
	return `
		...
		<text x="${x + 14}" y="${y + 28}" fill="${PURPLE}" font-family="Segoe UI,Arial,sans-serif" font-size="15" font-weight="800">VALORANT HACKS</text>
...
export function hacksBadge(w, h) {
	return `
		...
		<text x="${w - 160}" y="${h - 18}" fill="${PURPLE}" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="700">VALORANT HACKS</text>
	`;
}
...
export function buildOverlaySvg(width, height, preset) {
	...
	switch (preset) {
		case 'hero':
			...
			body += espBox(width * 0.71, height * 0.38, width * 0.08, height * 0.2, 'Spike · 31m', GREEN);
			body += aimbotOverlay(width, height);
			body += radarOverlay(width, height);
			break;
		case 'esp':
			body += espBox(width * 0.18, height * 0.25, width * 0.11, height * 0.32, 'Agent · 56m', RED, snap);
			body += espBox(width * 0.45, height * 0.18, width * 0.1, height * 0.28, 'Agent · 112m', RED, snap);
			body += espBox(width * 0.62, height * 0.32, width * 0.09, height * 0.24, 'Agent · 74m', RED, snap);
			body += espBox(width * 0.78, height * 0.4, width * 0.07, height * 0.18, 'Enemy · 95m', '#fbbf24');
			body += espBox(width * 0.35, height * 0.55, width * 0.06, height * 0.1, 'Spike · 41m', GREEN);
			break;
		case 'aimbot':
		...
		case 'wallhack':
			...
			break;
		case 'radar':
			body += espBox(width * 0.48, height * 0.26, width * 0.1, height * 0.26, 'Agent · 52m', RED, snap);
			body += radarOverlay(width, height);
			break;
		case 'menu':
			...
			break;
		case 'site':
			body += espBox(width * 0.42, height * 0.28, width * 0.14, height * 0.38, 'Site · 12m', RED, snap);
			body += aimbotOverlay(width, height);
			break;
		case 'deathmatch':
			...
			break;
		case 'spike':
			body += espBox(width * 0.3, height * 0.45, width * 0.07, height * 0.09, 'Spike · 28m', GREEN);
			body += espBox(width * 0.55, height * 0.38, width * 0.06, height * 0.08, 'Ability · 55m', GREEN);
			body += espBox(width * 0.68, height * 0.52, width * 0.06, height * 0.08, 'Spike timer · 0:38', '#fbbf24');
			body += espBox(width * 0.44, height * 0.22, width * 0.09, height * 0.24, 'Agent · 88m', RED, snap);
			break;
		case 'map':
			body += espBox(width * 0.35, height * 0.4, width * 0.08, height * 0.12, 'A Site', RED);
			body += espBox(width * 0.55, height * 0.32, width * 0.07, height * 0.1, 'Spike', GREEN);
			body += espBox(width * 0.62, height * 0.55, width * 0.09, height * 0.11, 'Ult', GREEN);
			break;
		default:
