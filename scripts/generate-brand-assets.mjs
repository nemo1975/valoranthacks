...
async function generateSiteLogo() {
	...
	await writeFile(path.join(imagesDir, 'valorant-hacks-logo.png'), logoPng);
	console.log('Wrote public/images/valorant-hacks-logo.png (512×512)');

	const logoWebp = await sharp(logoPng).webp({ quality: 90, effort: 6 }).toBuffer();
	await writeFile(path.join(imagesDir, 'valorant-hacks-logo.webp'), logoWebp);
	console.log('Wrote public/images/valorant-hacks-logo.webp');
}
...
async function generateWebManifest() {
	const manifest = {
		name: 'Valorant Hacks',
		short_name: 'Valorant Hacks',
		description: 'Undetected valorant hacks — ESP, aimbot, radar and for PC',
		start_url: '/',
		display: 'standalone',
